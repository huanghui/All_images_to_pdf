...
'''def get_files_sort(path,state):
    print("开始排序",state,"的文件，返回相对路径")
    jpg_files=[]

    for root,dirs,files in os.walk(path):
        for file in files:
            if file.find(state)==0:
...
def makeLastPdf(pdfFileName,path):
    print("排序图路径",path)
    images= state_sort(path)
    #print()
    pdf=FPDF()
...
###########对所有图片排序，并计数，以便后续逐一添加
    ...
    #pdf.add_page()
    #pdf.image(images[0],20,30,length,width)
    #print("pdf可以加入该文件",images[0])
